...
Download the executable for your platform from the [`win/`](./win) or [`linux/`](./linux) folder and run it directly — no installation required.
...
```bash
git clone https://github.com/Ezzyky/matrice.git
cd matrice
python main.py
```
...
Launch the app and pick an operation from the main menu:

```
[A] Opérations sur les matrices          [B] Réduction et résolution
    [1]. Addition                            [7]. Réduction de Gauss
    [2]. Soustraction                        [10]. Rang
    [3]. Multiplication
    [4]. Multiplication par scalaire
    [5]. Transposée Aᵀ
    [6]. la puissance d'une matrice

[C] Propriétés et analyse                [E] Outils
    [11]. Déterminant                        [23]. Historique
    [12]. Vérifier inversible                [24]. Infos
    [13]. Vérifier symétrique Trace          [25]. Home
    [14]. Vérifier diagonale
    [15]. Trace                              [0]. Quitter
    [16]. Inverse
```

You'll then be prompted for the matrix dimensions and its coefficients. After each result, a sub-menu lets you:

1. Save the result to a dated report file
2. View the session history
3. Read a detailed explanation of the operation (definition, formula, worked example)

> **Note:** the interface and prompts are in French; matrix coefficients can be integers or real numbers.
...
├── operations.py    # Core operations: addition, subtraction, multiplication, transpose, power
├── analyse.py        # Determinant, trace, inverse, diagonal check
├── gauss.py         # Gaussian elimination and rank
├── matrice_io.py     # Matrix input, validation, and display
├── utils.py          # History tracking, file export, post-operation menu
├── menu_txtes.py     # All menu, help, and info screens
