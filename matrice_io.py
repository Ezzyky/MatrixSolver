...
def prenant_matrice(
    nom: str = "A",
    flag: int = 0,
    matrice2: list[list[float]] = None,
    msg: str = "soustraction",
) -> list[list[float]]:
    ...
    # L'addition et la soustraction exigent deux matrices de même dimension.
        ...
        if len(matrice2) != lignes or len(matrice2[0]) != colonnes:
            raise dimensions(
                f"""-----> {msg} impossible — dimensions incompatibles ({(len(matrice2))}x{(len(matrice2[0]))} ≠ {(lignes)}x{(colonnes)})"""
            )
    # Le produit matriciel impose colonnes(A) = lignes(B).
    elif flag == 2:
        if len(matrice2[0]) != lignes:
            raise nombreLIGNESmemeDEcolonnes(
                "----> Multiplication impossible : le nombre de colonnes de la première matrice doit être égal au nombre de lignes de la deuxième."
            )
    ...
    # Cette variante vérifie deux matrices carrées de même ordre.
        ...
        if (
            lignes != len(matrice2) and colonnes != len(matrice2[0])
        ) or lignes != colonnes:
            raise carrees_ordre(
                f"----> Les matrices A et B doivent être carrées et de même ordre ({(len(matrice2))}x{(len(matrice2[0]))} ≠ {(lignes)}x{(colonnes)})."
            )
...
def Operations_gauss_affichage(
    matrice: list[int], ligne_pivot: int, col_pivot: int, flag: int = 0
) -> None:
    lignes = len(matrice)
    colonnes = len(matrice[0])
    if flag == 0:
